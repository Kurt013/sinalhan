...
<?php
    ...
    $staffbmis->view_unarchive_brgyclearance();
...
if (isset($_GET['id_clearance'])) {
    $id_clearance = $_GET['id_clearance']; // Get the 'report_id' parameter from the URL
    $doc_status = 'archived';

    // Prepare SQL query to fetch data based on the 'report_id'
    $stmt = $conn->prepare("SELECT * FROM tbl_clearance WHERE id_clearance = :id_clearance AND doc_status = :doc_status");
    $stmt->bindParam(':doc_status', $doc_status); // Bind the 'doc_status' as a string
    $stmt->bindParam(':id_clearance', $id_clearance); // Bind the 'report_id' as a string
    $stmt->execute();
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input class="form-control issueno" type="text" value="<?php echo htmlspecialchars($row['id_clearance']); ?>" readonly>
            </div>
        </div>
...
?>
    ...
    <a>
    <button class="btn btn-danger retrieve-btn archive-btn" type="button" style="width: 70px; font-size: 17px;" title="Retrieve" name="unarchive_clearance">  <i class="fas fa-undo"></i> </button>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    archiveBtn.addEventListener('click', function () {
        ...
        const dataId = document.querySelector('input[name="id_clearance"]').value;
        // Set the ID in the form
        archiveForm.querySelector('input[name="id_clearance"]').value = dataId;
