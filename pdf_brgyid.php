...
if (isset($_POST['views_data'])) {
    $columns = json_decode($_POST['views_data'], true); 
        ...
        $pdf->Cell(30, 10, 'Precinct No', 1, 0, 'C');
        $pdf->Cell(30, 10, 'Valid Until', 1, 0, 'C');

        // Add the rescert list
        $pdf->SetFont('helvetica', '', 12);
        foreach ($columns as $column) {
            $pdf->Cell(35, 10, $column['id_brgyid'], 1, 0, 'L');
            ...
            $pdf->Cell(30, 10, $column['bdate'], 1, 0, 'L');
            $pdf->Cell(30, 10, $column['status'], 1, 0, 'L');
            $pdf->Cell(30, 10, $column['precint_no'], 1, 0, 'L');
            $pdf->Cell(30, 10, $column['valid_until'], 1, 0, 'L');
        }
